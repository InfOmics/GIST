...
class GIST () :
    def __init__(self, 
        ...
        epochs=1100, 
        dim_hidden=32,
        dim_output=32, #20 for non visium
        random_seed = 35,
        is_visium='Visium'
        ):
        if is_visium == 'Visium':
        ...
        self.adata=create_graph(adata, is_visium=self.is_visium, seed=random_seed)
        ...
        if issparse(adata.X):
            self.data = pca(self.adata.X.toarray(),n_components=dim_output, random_state=self.random_seed) 
        else:
            self.data = pca(self.adata.X,n_components=dim_output, random_state=self.random_seed)  #if data already a matrix
    ...
    def train(self):
        ...
        self.model = GCN(self.dim_input, self.dim_hidden, self.dim_output).to(self.device).double()
        ...
        for epoch in tqdm(range(self.epochs), desc="Training Progress"):
            ...
            self.features, x_corrupted, self.adj, self.spatial_neigh, DGSItraining=True
            ) 
            ...
            loss = lambda_Recon*recon_loss + loss_RMI +  loss_SMI 
        ...
        with torch.no_grad():
            self.model.eval()
            emb = self.model(self.features, x_corrupted, self.adj, self.spatial_neigh, DGSItraining=False)
